...
public record AutomationSettings (int runs, int strategy) {
    public static AutomationSettings create() {
        int runs, strategy = -1;
        runs = FromStdin.getInt("Enter number of runs:", _ -> true, "Invalid input, try again.");
        while (strategy != 0 && strategy != 1) {
            strategy = FromStdin.getInt("Enter strategy (0 => stay, 1 => switch):", i -> i == 0 || i == 1, "Invalid input, try again.");
        }
